...
class PurchaseAdmin(CustomModelAdmin):
    inlines = [PurchaseItemInline]
    ordering = ['-date_created']
    search_fields = ['student__name', 'student__student_id']
    form = PurchaseForm
    date_hierarchy = 'date_created'
    list_filter = ['billable', 'purchaseitem__product']
    ...
    def get_readonly_fields(self, request, obj=None):
        ...
        fields = super(PurchaseAdmin, self).get_readonly_fields(request=request, obj=obj)
        if not self.has_save_permission(request):
    ...
    def get_queryset(self, request):
        ...
        """
        qs = super(PurchaseAdmin, self).get_queryset(request).annotate(
            total=ExpressionWrapper(Sum(F('purchaseitem__product__price') * F('purchaseitem__quantity')),
    ...
    def total(self, obj):
        return pretty_currency(obj.total or 0)
